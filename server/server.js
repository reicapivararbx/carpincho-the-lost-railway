...
import { registerRailsRoom, syncRailsRoom, closeRailsRoom } from './registry-bridge.js';
...
const notifyRegistry=async(room,kind='sync',extra={})=>{
  try{
    if(kind==='register') await registerRailsRoom(room,extra);
    else if(kind==='close') await closeRailsRoom(room,extra.reason||'empty');
    else await syncRailsRoom(room,extra);
  }catch{/* non-fatal */}
};
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(type==='CREATE_ROOM'){
      const room=authority.createRoom(p.id,payload);
      send(ws,'ROOM_CREATED',{code:room.code,hostId:room.hostId,private:room.private,serverId:room.portalServerId||null});
      notifyRegistry(room,'register',{
        hostName:p.name,
        hostUserId:payload.hostUserId||0,
        serverId:payload.serverId||payload.portalServerId||null,
      });
    }
    else if(type==='JOIN'){
      const result=authority.joinRoom(p.id,payload);
      if(!result.ok)return send(ws,'ERROR',{msg:result.reason});
      send(ws,'JOINED',{code:result.room.code,hostId:result.room.hostId,role:result.room.roles.get(p.id),world:result.room.world});
      broadcast(result.room,'PLAYER_JOINED',{playerId:p.id,name:p.name},ws);
      notifyRegistry(result.room,'sync');
    }
  ...
  ws.on('close',()=>{
    const room=authority.leave(ws.playerId);
    authority.savePlayer(ws.playerId);
    if(room){
      authority.saveRoom(room.code);
      broadcast(room,'PLAYER_LEFT',{playerId:ws.playerId,hostId:room.hostId});
      if(room._emptied) notifyRegistry(room,'close',{reason:'empty'});
      else notifyRegistry(room,'sync');
    }
